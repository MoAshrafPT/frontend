...
export default function Tools(){
    ...
    else if(localStorage.getItem('role') === 'admin' || localStorage.getItem('role') === 'manager'){
    ...
    useEffect(()=>{
        ...
        .then(res=> res.json())
        .then(data=>{setTools(data); console.log(data);
        })
    ...
    type FormData = {
      ...
      const [requestedDDL,setRequestedDDL] = useState<number>(new Date().getDate());

      const submitData = (data: FormData)=>{
        axios.post('http://localhost:8081/requesttools',{tool: data.toolName,quantity: data.quantity,memberID: Number(localStorage.getItem("userId")),deadline:data.requestDeadline})
        .then((res)=>{
            console.log(res);
      ...
      <div className="big-box">
        <form action="" className="form1" onSubmit={handleSubmit(submitData)}>
          <div className="mb-3">
            ...
            <div className="box">
              <input type="text" {...register("toolName")}
